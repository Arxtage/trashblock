...
A Chrome extension that blocks distracting websites with a typing challenge. No tracking, no accounts, and no data leaves your browser — just friction between you and your distractions.
...
- **Subdomain blocking** — Blocking `twitter.com` also blocks `mobile.twitter.com` and other `*.twitter.com` subdomains. Separate domains like `x.com` should be added separately.
...
- **Protected settings** — Removing a site from the block list, changing your phrase, or changing active days requires completing the typing challenge. No quick toggles to disable in a moment of weakness.
...
TrashBlock is private and local-first:

- It checks page URLs locally to decide when to show the block screen
- No browsing data or page content is sent anywhere
